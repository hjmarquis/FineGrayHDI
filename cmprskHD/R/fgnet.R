fgnet=function(x,is.sparse,is.tdep,ix,jx,y,ycr,cr,weights,offset,alpha,nobs,nvars,jd,vp,cl,ne,nx,nlam,flmin,ulam,thresh,isd,vnames,maxit)
{
  if(is.null(offset)){
    offset=rep(0,nobs)
    is.offset=FALSE}
  ...
  if(is.tdep)
  {
    if(!is.matrix(y)||!all(match(c("start","stop","status"),dimnames(y)[[2]],0)))
      stop("Fine-Gray model (with left-truncation or time-dependent covariates)
    ...
    fit=if(is.sparse) .Fortran("spfgLTnet",
                               parm=alpha,nobs,nvars,x,ix,jx,qy,ty,ycr,tevent,cr,offset,weights,jd,vp,cl,ne,nx,nlam,flmin,ulam,thresh,
    ...
    else .Fortran("fgLTnet",
                  parm=alpha,nobs,nvars,as.double(x),qy,ty,ycr,tevent,cr,offset,weights,jd,vp,cl,ne,nx,nlam,flmin,ulam,thresh,
                  maxit,isd,# need to get JHF to reverse these
                  lmu=integer(1),
                  ca=double(nx*nlam),
                  ia=integer(nx),
  ...
  }else
  {
    if(!is.matrix(y)||!all(match(c("time","status"),dimnames(y)[[2]],0)))
      stop("Fine-Gray model (without left-truncation or time-dependent covariates)
    ...
    fit=if(is.sparse) .Fortran("spfgnet",
                               parm=alpha,nobs,nvars,x,ix,jx,ty,tevent,cr,offset,weights,jd,vp,cl,ne,nx,nlam,flmin,ulam,thresh,
                               maxit,isd,# need to get JHF to reverse these
                               lmu=integer(1),
                               ca=double(nx*nlam),
                               ia=integer(nx),
                  ...
                  parm=alpha,nobs,nvars,as.double(x),ty,tevent,cr,offset,weights,jd,vp,cl,ne,nx,nlam,flmin,ulam,thresh,
                  maxit,isd,# need to get JHF to reverse these
                  lmu=integer(1),
                  ca=double(nx*nlam),
                  ia=integer(nx),
