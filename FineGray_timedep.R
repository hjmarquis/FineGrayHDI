...
FG.Surv=function(surv,cause,ftime, Z, id = 1:nrow(surv))
{
  n=nrow(surv)
  if(ncol(surv)==2)
  {
  ...
  # sort entries by id and survival intervals
  id.time.order = order(id,surv[,1])
  surv = surv[id.time.order,]
  cause = cause[id.time.order]
  Z = Z[id.time.order,]
  id = id[id.time.order]
  ...
  tdep = sum(cause[last.row]>0) < sum(cause>0)
  if(tdep)
  {
    if(missing(ftime) | is.null(ftime))
      stop("Must provide ftime when covariates change beyond the type-2 events.")
    ftime = ftime[id.time.order]
    
    C.Surv = Surv(surv[last.row, 2], cause[last.row]==0)
  }else{
  ...
  for(i in 1:length(id.unique))
  {
    ...
    if( ni > 1)
    {
      if(any(cause[id.list[[i]]]==1) & any(c(id.surv[,3]) != c(rep(0, ni-1),1)))
        stop(paste( "Wrong event indication for id = ",
                    id.unique[i],
          ". Only the last row for each id may have type-1 event. "
          ,sep=''))
    ...
    if(cause[last.row[i]] > 1)
    {
      if(tdep)
      {
        Xft = ftime[last.row[i]]
        ck = C.knot[C.knot > Xft]
        nk=length(ck)
        omega = Ghat(ck)/Ghat(X)
        
        wknot = unique(c(id.surv[,1],Xft,ck))
        nknot = length(wknot)
        k.cov = rep(1:0,c(ni,nknot-ni))
        t.ord = order(wknot)
        wknot = wknot[t.ord]
        k.cov = cumsum(k.cov[t.ord])[-nknot]
        i.start = wknot[-nknot]
        i.end = wknot[-1]
        
        IPW.weight = rep(1,nknot-1)
        IPW.weight[i.start > Xft] =Ghat(i.start[i.start>Xft])/Ghat(Xft)
        
        FG.start = c(FG.start, i.start)
        FG.end = c(FG.end, i.end)
        FG.event = c(FG.event, rep(0, nknot-1))
        FG.weight = c(FG.weight, IPW.weight)
        FG.id = c(FG.id, rep(id.unique[i],nknot-1))
        FG.Z = c(FG.Z, id.Z[k.cov])
        FG.X = c(FG.X, rep(Xft,nknot-1))
      }else{
        ck = C.knot[C.knot > X]
        nk=length(ck)
        omega = Ghat(ck)/Ghat(X)
